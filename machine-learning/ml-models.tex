...
Describe the ROC curve; \begin{itemize} \item Illustrates the diagnostic ability of a binary classifier as its discrimination threshold is varied. \item Plots true pos rate against false positive rate at various threshold settings. \end{itemize} 

What is type I error?; False positives. (think: 1: first place, fp.) 

What is type II error?; False negatives. (think: 2: DOUBLE negative, false negative.)
...
Describe how you would carry out a t-test to check if a pop mean is equal to some value $\mu$.; \begin{itemize} \item Calculate T-stat: $t=Z/s = (X-\mu)/\frac{s}{\sqrt(n)}$. \item Degrees of freedom = n-1 (one less because avg needs to equal mean used in t stat calc) \item see if t-stat is in e.g. 95\% confidence interval of t-dist with this DoF. \end{itemize}

T-statistic for testing if two populations have the same mean (assume same unknown standard deviation); \begin{itemize} \item $t=\frac{\bar{x}_1 - \bar{x}_2}{\sqrt{\frac{s_p^2}{n_1} + \frac{s_p^2}{n_2}}}$. \end{itemize}

Two examples of applications of F-test; Test whether the: \begin{itemize} \item Means of a given set of normally distributed pops, all with the same sd, are equal. \item Proposed regression model fits the data well. \end{itemize}

Describe how to test if more than two populations have the same mean; ANOVA \begin{itemize} \item F-ratio: Between-group var/Within-group var. $\frac{MS_{between}}{MS_{within}} = \frac{n\sum_k (x_k-x_G)^2/(k-1)}{\sum_{i, k}(x_i-x_k)^2/(N-k)}$ \item $x_G$ is mean of means, n is num datapoints per sample, N total num datapoints. \item F-dist sort of like chi squared, never negative, peaks at 1 (between and within group var the same when things differ by chance). \item Non-directional test (unlike t-, Z-tests.) \end{itemize}
...
Three ways of calculating (implementing) analytical solution for linear regression; \begin{itemize} \item Recall analytical solution to $Xw = y$ is $w = (X^TX)^{-1}X^Ty$. \item Can solve directly \item Can use QR decomposition: $w=R^{-1}Q^Ty$ \begin{itemize} \item If X is mxn, \item Q is mxm orthogonal, R is mxn upper triangular. \end{itemize} \item Can use SVD: $w=VS^{-1}U^Ty$. \end{itemize}

Describe QR factorisation of a matrix; \begin{itemize} \item If matrix to be factorised is $X_{m\times n}=QR, m\geq n$, \item Q is orthogonal mxm, \item R is upper triangular mxn \item last $m-n$ rows of R are zero so can simplify further.  \end{itemize}
...
List three (arguably four) main types of loss functions you might use.; \begin{itemize} \item (Negative) Likelihood loss: $-p(\mathbf{x})$. \item Negative Log likelihood: Cross entropy loss / log loss: $-\sum_i y_i\log(\hat{p}_i)$ \item L1 \ite L2 \end{itemize}
